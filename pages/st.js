...
function renderTrackSelect() {
    const select = document.getElementById('trackSelect');
    if (!select) return;
    select.innerHTML = '<option value="">Выберите музыкальную композицию</option>';
...
function loadMusicFiles() {
    ...
    tracks = [
        { name: 'Священная война', path: 'redMusic/HolyWar.mp3' },
        { name: 'Три танскиста', path: 'redMusic/TriTankista.mp3' },
        { name: 'Катюша', path: "redMusic/Katyusha.mp3" },
        { name: 'Смуглянка', path: "redMusic/Smuglanka.m4a" }
